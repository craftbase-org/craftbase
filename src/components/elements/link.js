...
function Link(props) {
  ...
  console.log("CONDITION", props, lastAddedElement);
  if (status === "construct" || lastAddedElement.id === props.id) {
    // Calculate x and y through dividing width and height by 2 or vice versa
    // if x and y are given then multiply width and height into 2
    const offsetHeight = 0;
    const prevX = localStorage.getItem("link_coordX");
    const prevY = localStorage.getItem("link_coordY");
      ...
      interact(`#${group.id}`).draggable({
        ...
        listeners: {
          ...
          move(event) {
            ...
            localStorage.setItem("link_coordX", parseInt(event.pageX));
